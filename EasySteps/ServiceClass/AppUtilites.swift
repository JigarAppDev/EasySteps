...
class AppUtilities{
    ...
    func getUserToken() -> JSON {
        guard let userDetail = UserDefaults.standard.value(forKey: "token") as? Data else { return JSON.init() }
        let data = JSON(userDetail)
        print(data)
        return data
    }
    ...
    //MARK: - Loader and net connectivity
    
    func getServiceList() -> NSArray {
        let encodedArray = Defaults.data(forKey: "ServiceList")
        //Decoding the Array
        let decodedArray = NSKeyedUnarchiver.unarchiveObject(with: encodedArray!) as! NSArray
        return decodedArray
    }
    ...
    func animateZoomforCell(zoomButton : UIButton)
    {
        UIView.animate(
            withDuration: 0.2,
            delay: 0,
            options: UIView.AnimationOptions.curveEaseOut,
            animations: {
                zoomButton.transform = CGAffineTransform.init(scaleX: 1.5, y: 1.5)
        },
            completion: nil)
    }
    
    func animateZoomforCellremove(zoomButton : UIButton)
    {
        UIView.animate(
            withDuration: 0.2,
            delay: 0,
            options: UIView.AnimationOptions.curveEaseOut,
            animations: {
                zoomButton.transform = CGAffineTransform.init(scaleX: 1.0, y: 1.0)
        },
            completion: nil)
    ...
    func getProfile(){
        let dic : JSON =  AppUtilities.sharedInstance.getUserDetail()
        let successed = {(responseObject: AnyObject) -> Void in
            if(responseObject.value(forKeyPath: "flag") as! NSNumber == 1){
                if let str = responseObject.value(forKeyPath: "data") {
                    userData = JSON.init(str)
                }
            }else{
                KSToastView.ks_showToast(responseObject.value(forKeyPath: "msg"), duration: ToastDuration)
            }
        }
        let failure = {(error: AnyObject) -> Void in
            KSToastView.ks_showToast(WrongMsg, duration: ToastDuration)
        }
        service.getWebServiceCall("profiles/view/\(dic["data"]["id"].stringValue).json", params: nil, isShowLoader: true, success: successed, failure: failure)
    }
